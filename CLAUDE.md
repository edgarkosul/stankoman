...
- **Потолки — все в `ChatAbuseGuard`**, и он единственная часть чата, проверяемая без базы
  и без сети: числа из `ai_support.chat.abuse`, в коде ни одного. Исходов четыре, и путать
  их нельзя: краулеру тишина (сообщение об ошибке — подсказка, как обойти проверку),
  человеку объяснение, а **исчерпанному дневному бюджету — менеджер**: вопрос остаётся
  в ленте и уходит штатной эскалацией, потому что покупатель ни в чём не виноват.
  Счётчики поднимает `remember()` у принятого сообщения, а не проверка. Сотруднику панели
  потолки не считаются (кроме бюджета — ключ шлюза общий с turbo). Сутки бюджета считаются
  по Москве: `app.timezone` здесь UTC, и иначе потолок обнулялся бы в три ночи. Израсходованное
  за сегодня видно плиткой в виджете расхода над «Диалогами» — больше нигде.
- **Капча — на первом сообщении разговора** (закрытый оператором диалог считается за новый),
  дальше посетителя удостоверяет кука. Провайдер, ключи и рубильник — `config/captcha.php`
  и `App\Services\Captcha\CaptchaManager`, **один на весь магазин**: у донора три копии
  этого решения разошлись. Неполная настройка = выключенная капча; недоступный сервис
  пропускает посетителя и пишет в лог (над капчей есть и намордник, и nginx). На деве
  и в тестах — заглушка: ключи выписаны на `intertooler.ru` с проверкой домена, и токен
  с `localhost` сервис не примет, то есть живого виджета на деве нет.
  Прятать угловой блок сервиса можно только в обмен на свою плашку (`x-captcha.notice`) —
  это условия использования, а не косметика.
- **Потолок на действиях Livewire** — `throttle:120,1`, ставится
  `AppServiceProvider::throttleLivewireUpdates()` через `Livewire::setUpdateRoute()`.
  Путь берётся у самого Livewire: с четвёртой версии это `/livewire-<хэш от APP_KEY>/update`,
  и свой литерал завёл бы ВТОРОЙ маршрут рядом с настоящим — сайт работал бы, а потолка
  не было бы ни на одном (сторож — `tests/Feature/LivewireUpdateThrottleTest.php`).
  Ниже него зоны `limit_req` в nginx на тот же эндпоинт и на `/chat/unread`
  ([`scripts/deploy/nginx/README.md`](scripts/deploy/nginx/README.md)): POST без CSRF-токена
  Laravel отбивает 419 **до** своего `throttle`, то есть его счётчик такой поток не видит
  вовсе, а воркер php-fpm тратится.
