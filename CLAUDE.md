...
Так же намеренно там стоят `AI_EMBEDDING_FAKE=true` и пустой `AI_GATEWAY_KEY`: `.env`
читается и в тестах, а ключ шлюза в нём живой и общий с turbo — тест, дошедший до
настоящего клиента, тратил бы чужие деньги. И ещё одна ловушка ассистента: джобы на
