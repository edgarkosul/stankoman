...
**Индексов в Meilisearch два.** Витринный `stankoman_products` — тот, что выше;
второй, `products_semantic`, — зеркало для смыслового поиска бота
(`App\Services\Catalog\CatalogSemanticIndex`, ночная команда `ai:catalog-embed`).
⚠️ **Эмбеддер объявлен только у зеркала, и переносить его на витринный индекс
нельзя**: Meilisearch отвергает документ без вектора целиком, вместе со всей
пачкой, — Scout шлёт документы пачками и о векторах не знает, так что любая
новинка импорта выпадала бы из поиска САЙТА до ближайшей ночи, а `scout:import`
падал бы весь. Поисковые и фильтруемые поля зеркало берёт из `config/scout.php`,
своей копии списка не держит.
...
**Смысл поверх слов** — зеркало `products_semantic`: вектор запроса считается
по ОРИГИНАЛЬНОЙ формулировке (смысл живёт в словах, а не в их записи), слова
ищутся нормализованными, гибрид отдаёт и то и другое одним запросом вместе
с фильтрами. Три ловушки, за каждую уже заплачено:

- **доля смысла у обозначений — 0.2, а не 0.5** (`semantic_ratio_model`):
  вектор выдуманного слова — шум с «сходством» 0.75–0.78, выше цены точного
  попадания с опечаткой (0.494), и на половине ранга он побеждал. Бренд — такое же
  обозначение, как артикул: решает это `CatalogBrands`, а не индекс, и везёт признак
  в `ProductQuery::$designation`. Любую правку долей проверять именно опечатанным
  брендом и артикулом, а не «компрессором для гаража», на котором всегда хорошо;
- **у гибрида не бывает пустой выдачи** — вектор всегда назовёт ближайших соседей.
  Поэтому слова проверяются отдельной пробой (`ProductTextSearch::run(probeAlways:)`),
  и если их в каталоге нет, выдача помечается как подобранная по смыслу. Без этого
  бот подавал бы шум вектора как точный ответ;
- **поля зеркала стареют**: `ai:catalog-embed` переписывает документ только у товаров
  с изменившимся ТЕКСТОМ (`content_hash`), а цена и наличие остаются снимком. Поэтому
  в 00:30, следом за `products:sync-currency-rates`, идёт `ai:catalog-embed --fields-only` —
  вызовов шлюза он не делает вовсе. Ценой бот не соврёт (её он читает из базы), но
  фильтры «до 40 тысяч» и «в наличии» без этого прохода работали бы по вчерашнему дню.

Зеркало собирается ночью в 03:00; до первого прогона и при сломанном зеркале бот ищет
по одним словам — это рабочий режим, а не поломка. Состояние показывает `ai:kb-doctor`
разделом «Смысловой поиск»: сколько документов, той ли моделью считаны векторы,
объявлен ли эмбеддер.
