...
class RunMetaltecProductImportJob implements ShouldQueue
{
    ...
    {
        return [
            (new WithoutOverlapping('catalog_import_metaltec'))
                ->releaseAfter(30)
                ->expireAfter($this->timeout + 60),
        ];
    }

    public function handle(MetaltecProductImportService $service, ImportRunOrchestrator $runs): void
    {
        $run = ImportRun::query()->find($this->runId);

        if (! $run) {
            Log::warning('Queued metaltec import run not found', [
                'run_id' => $this->runId,
        ...
        try {
            ...
            foreach (($result['url_errors'] ?? []) as $rowError) {
                $run->issues()->create([
                    'row_index' => null,
                    'code' => 'parse_error',
                    'severity' => 'error',
                    'message' => (string) ($rowError['message'] ?? 'Unknown parse error.'),
                    'row_snapshot' => [
                        'row' => (string) ($rowError['url'] ?? ''),
                    ],
                ]);
            }
