...
<div class="z-30 w-full {{ $rootClass }}" x-data x-on:click.stop.prevent>
    @if ($isProductLayout)
        <div class="inline-flex h-12 items-center justify-between border px-3 shadow-sm transition {{ $quantityShellClass }}">
            <button
                type="button"
                wire:click="decrementQty"
                wire:loading.attr="disabled"
                wire:target="decrementQty,incrementQty,add"
                @disabled($isDisabled)
        ...
        <button
            ...
            @endif
            wire:loading.attr="disabled"
            wire:target="add,decrementQty,incrementQty"
            @disabled($isDisabled)
        ...
        <button
            ...
            @endif
            wire:loading.attr="disabled"
            wire:target="openOneClickOrder,decrementQty,incrementQty"
            @disabled($isDisabled)
            aria-disabled="{{ $isDisabled ? 'true' : 'false' }}"
