...
<section class="bg-zinc-200 py-10">
    <div class="mx-auto w-full max-w-6xl px-6">
        <div class="mb-4 flex items-start justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-brand-gray">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div class="lg:col-span-2 border border-zinc-300 bg-white">
                <div class="border-b border-zinc-200 px-4 py-3 text-lg font-semibold text-brand-gray">
                    Состав заказа
                </div>
                ...
                <div class="divide-y divide-zinc-200">
                    @forelse ($order->items as $item)
                        <div class="grid grid-cols-1 gap-3 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                            <div>
                                @if ($item->product?->slug)
                                    <a
                                        ...
                                        class="font-semibold text-brand-gray hover:text-brand-red"
                                    >
                                        {{ $item->name ?? ($item->product?->name ?? 'Товар') }}
                                    </a>
                                @else
                                    <div class="font-semibold text-brand-gray">
                                        {{ $item->name ?? ($item->product?->name ?? 'Товар') }}
                                    </div>
                                @endif
                            ...
                            <div class="text-right text-lg font-bold text-brand-gray">
            ...
            <aside class="h-max border border-zinc-300 bg-white p-4">
                <h2 class="text-lg font-semibold text-brand-gray">Итого</h2>

                <dl class="mt-3 space-y-2 text-sm">
                    <div class="flex items-center justify-between gap-3">
                        <dt class="text-zinc-600">Товары</dt>
                        <dd class="font-semibold text-brand-gray">{{ price($order->items_subtotal ?? 0) }}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        <dt class="text-zinc-600">Скидка</dt>
                        <dd class="font-semibold text-brand-gray">-{{ price($order->discount_total ?? 0) }}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                        <dt class="text-zinc-600">Доставка</dt>
                        <dd class="font-semibold text-brand-gray">{{ price($order->shipping_total ?? 0) }}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-3 border-t border-zinc-200 pt-2 text-base">
                        <dt class="font-semibold text-brand-gray">К оплате</dt>
                        <dd class="text-xl font-bold text-brand-gray">{{ price($order->grand_total ?? 0) }}</dd>
                    </div>
                </dl>
            </aside>
        </div>
