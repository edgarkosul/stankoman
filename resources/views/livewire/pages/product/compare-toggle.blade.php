...
<div x-data x-on:click.stop.prevent class="flex items-end gap-2 overflow-hidden z-30">
    ...
    >
        <x-icon name="compare" class="{{ $iconSizeClass }} {{ $iconToneClass }}" />

        @if ($isCompareVariant)
            <span class="whitespace-nowrap text-zinc-800 hover:text-brand-red hidden md:block">
                {{ $added ? 'В сравнении' : 'Сравнить' }}
            </span>
        @endif
    </button>
    ...
    @if ($showLink)
        <a
            ...
            title="Перейти в сравнения"
            wire:click.stop
            class="text-xs underline cursor-pointer hidden xs:block"
        >
            В сравнения
        </a>
    @endif
