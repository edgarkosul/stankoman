...
it('знает режим работы — тот же, что в шапке сайта', function () use ($requisites): void {
    expect($requisites()->text)->toContain(
        "## Режим работы\n\n"
        ."Пн – Пт: 9:00 – 18:00\n"
        ."Сб – Вс: выходной\n"
        .'В субботу и воскресенье — отгрузка по предварительной договорённости.'
    );
});

it('не отдаёт боту незаполненный режим работы как «всегда выходной»', function () use ($requisites): void {
    config(['company.work_schedule' => ['days' => [], 'note' => '']]);

    expect($requisites()->text)->not->toContain('Режим работы');
});
...
it('режется чанкером по разделам, чтобы вопрос про БИК находил банковский фрагмент', function () use ($requisites): void {
    ...
    expect(array_column($chunks, 'section_path'))
        // Отдельного раздела «НДС» нет намеренно: одна строка с цифрами притягивала
        // посторонние вопросы про числа — ставка лежит в «Организации».
        ->toBe([['Организация'], ['Банковские реквизиты'], ['Контакты'], ['Режим работы']])
